...
export const getComment = async(req:Request,res:Response)=>{
  try {
    const tweet_id = req.params.tweet_id;
    const [result] = await db.query<Comments[]>(`select * from comments where tweet_id = ?`,[tweet_id]);
...
export const deleteComment = async(req:Request,res:Response)=>{
  try {
    ...
    const result = await db.query<ResultSetHeader>(`delete from comments where comment_id = ?`,[comment_id]);
