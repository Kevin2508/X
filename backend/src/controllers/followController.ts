...
export const followUser = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const result = await db.query<ResultSetHeader>(`insert into follows(follower_id,followee_id) values(?,?)`,[follower_id,followee_id]);
...
export const unfollowUser = async (
  ...
  res: Response,
    ...
    const follower_id = req.user?.user_id;
    const followee_id = req.params.followee_id;
    const result = await db.query<ResultSetHeader>(`delete from follows where follower_id = ? and followee_id = ?`,[follower_id,followee_id]);
